...
///////////////
// void State::computeNetMoment( double& xforce, double& yforce) const{
//     // zmoment = 0.0;
//
//     for (int j = 0;j < f.getNumPoints(); j++) {
//       //zM = x.f(0, j) * grid.Dx() * grid.Dx() * (Boundary.f(1, j) - yC) * sin(theta) + x.f(1, j) * grid.Dx() * grid.Dx() * (Boundary.f(0, j) - xC) * cos(theta) + zM;
//       // zmoment = f(Y,j) * omega.Dx() * omega.Dx() * (Boundary.f(0, j) - xC) + zmoment;
//
//     }
// }

void State::computeNetMoment( double& xC, const State& Boundary, double& zmoment) const{
    zmoment = 0.0;
