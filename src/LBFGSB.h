...
public:
    ...
    inline int minimize(Foo& f, Vector& x, Scalar& fx, const Vector& lb, const Vector& ub, const std::string& save_dir)
    {
        using std::abs;
        ...
        ///////////////////////////////////////
        // customized for data saving
        double drag[m_param.max_iterations];
        ///////////////////////////////////////
        ...
        for( ; ; )
        {
            ...
            drag[k-1] = fx;
            // create drag
            std::ifstream file_Drag("../examples/" + save_dir + "Drag.txt");
            double D_temp[m_param.max_iterations];
            // Update Error
            for (int i = 0; i < m_param.max_iterations; ++i)
              {
                file_Drag >> D_temp[i];
              }
            // Save drag
            std::ofstream myfile_Drag("../examples/" + save_dir + "Drag.txt");
            if (myfile_Drag.is_open())
            {
              // for (int count = 0; count < size; count++) {
              for (int count = 0; count < m_param.max_iterations; ++count) {
                myfile_Drag << drag[count] << " ";
              }
              myfile_Drag.close();
            }
